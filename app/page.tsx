...
function Hero({ searchOpen, setSearchOpen, searchQuery, setSearchQuery, onSearchSubmit }: {
  ...
  // Live, honest feedback: counts phones in the selected price band via the
  // real search endpoint. Priorities affect ranking, not filtering, so they
  // are surfaced as a label next to the count rather than folded into it.
  useEffect(() => {
    if (!tierId) { setMatchCount(null); return }
    const tier = PRICE_TIERS.find(t => t.id === tierId)
    if (!tier) return
    const controller = new AbortController()
    setMatchLoading(true)
    api.phones.search({ min_price: tier.min, max_price: tier.max, page_size: 1 }, controller.signal)
      .then(d => setMatchCount(d.total))
      .catch(err => {
      ...
      <div className="hero-grid" style={{
        ...
        <div className="hero-left">
          ...
          <div className="hero-fade" style={{ animationDelay: '190ms', display: 'flex', gap: 0, marginBottom: 44, flexWrap: 'wrap' }}>
            {PROOF_STATS.map((stat, i) => (
              <div key={stat.label} style={{
                ...
                <div style={{
                  fontFamily: f.serif, fontSize: 26, letterSpacing: '-0.5px', marginBottom: 2,
                  color: stat.label === 'sponsored picks' ? c.accent : '#fff',
                }}>
                  {stat.value}
                </div>
          ...
          <div className="hero-fade" style={{ animationDelay: '250ms' }}>
            {/* Desktop: always visible, no extra click for a high-intent visitor */}
            <form onSubmit={onSearchSubmit} className="search-desktop-inline" style={{ position: 'relative', maxWidth: 340 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 8 }}>
            ...
            {/* Mobile: collapsed behind a toggle to save vertical space */}
        ...
        <div className="hero-fade hero-console" style={{
          ...
          <div style={{ fontSize: 11.5, color: c.accent, fontWeight: 600, marginBottom: 14, letterSpacing: '0.2px' }}>
            No affiliate bias — ranked on specs and price only
          </div>
          ...
          <div className="tier-segmented" style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.12)', marginBottom: tierId ? 10 : 22 }}>
          ...
          {tierId && (
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', marginBottom: 20, minHeight: 16 }}>
              {matchLoading
                ? 'Counting matches…'
                : matchCount != null
                  ? `${matchCount.toLocaleString()} phone${matchCount !== 1 ? 's' : ''} in this range`
                  : null}
            </div>
          )}
            ...
            <div style={{ animation: 'fadeIn 250ms ease' }}>
              ...
              <div style={{ marginTop: 28 }}>
                ...
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', textAlign: 'center', marginTop: 10 }}>
                  20 seconds · no signup · independent ranking
                </div>
              </div>
      ...
      <div className="rankings-rail" style={{ borderTop: `1px solid ${c.border}` }}>
        {Object.entries(CATEGORY_META).map(([slug, meta], i) => (
          ...
          >
            ...
            <span className="rankings-rail-desc" style={{ fontSize: 13, color: c.text3, flex: 1 }}>{meta.desc}</span>
  ...
  return (
    <div style={{ minHeight: '100vh', background: c.bg }}>
      ...
      <style>{`
        ...
        .search-mobile-toggle { display: none; }

        ${mq.xl} {
          .hero-grid { grid-template-columns: 1fr !important; }
          .hero-console { max-width: 460px; order: 1; }
          .hero-left { order: 2; }
        }
        ${mq.lg} {
          ...
          .search-desktop-inline { display: none; }
          .search-mobile-toggle { display: block; }
        }
        ...
        ${mq.sm} {
          ...
          .rankings-rail-desc { display: none; }
        }
