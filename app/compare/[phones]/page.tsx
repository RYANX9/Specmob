...
export default async function CompareWithPhonesPage({ params }: PageProps) {
  ...
  const { phones: validPhones, verdict, winnerId } = await resolveComparePhones(slugParts)
  ...
  return (
    <>
      ...
      />
      <CompareClient initialPhones={validPhones} initialVerdict={verdict} initialWinnerId={winnerId} />
    </>
  )
}
