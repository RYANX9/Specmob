...
import { getTierStyle, getChipsetTierLabel } from '@/lib/tiers'
...
function FilterChips({ filters, onChange }: { filters: SearchFilters; onChange: (f: SearchFilters) => void }) {
  ...
  if (filters.chipset_tier)   chips.push({ label: getChipsetTierLabel(filters.chipset_tier), clear: () => onChange({ ...filters, chipset_tier: undefined }) })
...
function HomeContent({ initialTrending, initialStats }: HomeClientProps) {
  ...
  const handleFiltersChange = (f: SearchFilters) => { analytics.filterApply(f); setFilters(f); setPage(1); commit(f, 1, sortIdx) }
