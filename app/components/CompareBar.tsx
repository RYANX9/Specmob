...
// Two call patterns exist in this app:
//   <CompareBar />                                       — home/brand pages, driven by the global useCompare() store
//   <CompareBar phones={...} onRemove={...} onClear={...} /> — phone detail, /pick, /best/[category], CompareClient,
//                                                              each of which keeps its own local comparePhones state
// The props are optional overrides. When phones is passed, the bar renders that
// page's local list; otherwise it falls back to the shared store. Previously this
// component ignored the props entirely and always read the shared store, so any
// page tracking compare state locally would toast "Added to compare" while the
// bar itself stayed empty.
...
export default function CompareBar({ phones: phonesProp, onRemove, onClear }: CompareBarProps = {}) {
  const router = useRouter()
  const store = useCompare()

  const isControlled = phonesProp !== undefined
  const phones = isControlled ? phonesProp : store.phones
  const remove = isControlled ? (onRemove ?? (() => {})) : store.remove
  const clear = isControlled ? (onClear ?? (() => {})) : store.clear
