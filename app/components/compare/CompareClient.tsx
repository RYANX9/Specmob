...
import { VERDICTS, getBestIdx, pickWinnerId, type VerdictItem } from '@/lib/compare/verdictConfig'
...
function displayName(p: Phone): string {
  const brand = (p.brand || '').trim()
  const name  = (p.model_name || '').trim()
  return brand && name.toLowerCase().startsWith(brand.toLowerCase()) ? name : `${brand} ${name}`.trim()
}

function setKeyOf(ps: Phone[]): string {
  return ps.map(p => p.id).sort((a, b) => a - b).join(',')
}

function shimmer(width: string, height: number, dark: boolean): React.CSSProperties {
  const a = dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'
  const b = dark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.10)'
  return {
...
function VerdictSkeleton() {
  return (
    <div aria-busy="true" aria-label="Writing our verdict" style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20, maxWidth: 480 }}>
      <div style={shimmer('100%', 13, true)} />
      <div style={shimmer('94%', 13, true)} />
      <div style={shimmer('82%', 13, true)} />
      <div style={shimmer('58%', 13, true)} />
    </div>
  )
}
        ...
        {phone.amazon_link && (
          <a
            ...
            onClick={() => analytics.affiliateClick({
              phone_id: phone.id,
              phone_name: displayName(phone),
              retailer: isAmazon ? 'amazon' : undefined,
              price: phone.price_usd ?? undefined,
              location: 'compare_column',
            })}
            target="_blank"
...
function VerdictHero({ phones, verdict, winnerId, loading }: {
  phones: Phone[]
  verdict: CompareVerdict | null
  winnerId: number | null
  loading: boolean
}) {
  const scores  = phones.map(p => resolveValueScore(p).score).sort((a, b) => b - a)
  const winner  = phones.find(p => p.id === winnerId) ?? phones[0]
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            {winner.amazon_link ? (
              <a
                ...
                onClick={() => analytics.affiliateClick({
                  phone_id: winner.id,
                  phone_name: displayName(winner),
                  retailer: isAmazon ? 'amazon' : undefined,
                  price: winner.price_usd ?? undefined,
              ...
              >
                Buy the {displayName(winner)} <ArrowRight size={15} />
              </a>
            ) : (
              <Link
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: `${LABEL_W + phones.length * 120}px`, tableLayout: 'fixed' }}>
          ...
          {SPEC_SECTIONS.map(section => (
            <tbody key={section.title}>
              ...
              {section.rows.filter(row => phones.some(p => row.getValue(p) !== '—')).map((row, rowIdx) => {
...
function BottomLine({ phones, verdict, winnerId, loading }: {
  phones: Phone[]
  verdict: CompareVerdict | null
  winnerId: number | null
  loading: boolean
  ...
  if (loading && !verdict) {
    return (
      <div aria-busy="true" style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: r.lg, padding: '28px 32px', textAlign: 'center', marginBottom: 40 }}>
        <h2 style={{ fontFamily: f.serif, fontSize: 24, color: c.text1, marginBottom: 6 }}>Who Each Phone Is Really For</h2>
        <p style={{ fontSize: 13, color: c.text3, marginBottom: 24 }}>Putting this together...</p>
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(phones.length, 3)},1fr)`, gap: 12 }} className="bottom-recs">
          {phones.slice(0, 3).map(p => <div key={p.id} style={{ ...shimmer('100%', 88, false), borderRadius: 10 }} />)}
        </div>
      </div>
    )
  }
  ...
  const bestValue  = phones.find(p => p.id === winnerId) ?? phones.reduce((a, b) => getScore(a) > getScore(b) ? a : b)
  const cheapest   = phones.reduce((a, b) => (a.price_usd ?? Infinity) < (b.price_usd ?? Infinity) ? a : b)
...
function CompareContent({
  initialPhones, initialVerdict, initialWinnerId, suggestedPairs,
}: {
  initialPhones: Phone[]
  ...
  const [serverWinner, setServerWinner] = useState<{ key: string; id: number } | null>(
    initialWinnerId != null ? { key: setKeyOf(initialPhones), id: initialWinnerId } : null,
  )
  const [verdictState, setVerdictState] = useState<{ key: string; verdict: CompareVerdict } | null>(
    initialVerdict ? { key: setKeyOf(initialPhones), verdict: initialVerdict } : null,
  )
  const [verdictLoading, setVerdictLoading] = useState(false)

  const currentKey = setKeyOf(phones)
  const verdict = verdictState && verdictState.key === currentKey ? verdictState.verdict : null
  const activeWinnerId = serverWinner && serverWinner.key === currentKey
    ? serverWinner.id
    : pickWinnerId(phones, p => resolveValueScore(p).score)
    ...
    api.phones.compare(idList)
      .then(data => {
        ...
        if (data.phones?.length) {
          ...
          const finalPhones = ordered.length ? ordered : data.phones
          const dataKey = setKeyOf(finalPhones)
          setPhones(finalPhones)
          setServerWinner(data.winner_id != null ? { key: dataKey, id: data.winner_id } : null)
          setVerdictState(data.verdict ? { key: dataKey, verdict: data.verdict } : null)
  ...
  useEffect(() => {
    if (phones.length < 2 || hydratedKey.current === phoneIdsKey) return

    const ids = phones.map(p => p.id)
    const reqKey = setKeyOf(phones)
    let cancelled = false
    ...
    api.phones.compare(ids)
      .then(data => {
        ...
        setServerWinner(data.winner_id != null ? { key: reqKey, id: data.winner_id } : null)
        if (data.verdict) setVerdictState({ key: reqKey, verdict: data.verdict })
  ...
  useEffect(() => {
    if (phones.length < 2 || hasVerdict) { setVerdictLoading(false); return }

    const reqKey = setKeyOf(phones)
    let cancelled = false
    setVerdictLoading(true)

    api.phones.compareVerdict(phones.map(p => p.id))
      .then(res => {
        if (cancelled) return
        if (res.winner_id != null) setServerWinner({ key: reqKey, id: res.winner_id })
        if (res.verdict) setVerdictState({ key: reqKey, verdict: res.verdict })
      })
      .catch(() => {})
      .finally(() => { if (!cancelled) setVerdictLoading(false) })

    return () => { cancelled = true }
  }, [currentKey, hasVerdict, phoneIdsKey])

  const viewedKey = useRef<string | null>(null)
  useEffect(() => {
    if (phones.length < 2 || viewedKey.current === phoneIdsKey) return
    viewedKey.current = phoneIdsKey
    analytics.compareView(phones)
  }, [phoneIdsKey])
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 24px 80px' }}>
        ...
        {hasPhones && !loading && (
          <>
            ...
            >
              {phones.map((p, i) => (
                <PhoneColumn key={p.id} phone={p} onRemove={() => handleRemove(p.id)} isWinner={phones.length >= 2 && p.id === activeWinnerId} />
              ))}
            ...
            {canCompare && (
              <>
                <VerdictHero phones={phones} verdict={verdict} winnerId={activeWinnerId} loading={verdictLoading} />
                <CategoryBreakdown phones={phones} />
                <SpecTable phones={phones} />
                <AdSlot placement="inline" />
                <BottomLine phones={phones} verdict={verdict} winnerId={activeWinnerId} loading={verdictLoading} />
              </>
            )}
...
export default function CompareClient({
  ...
  return (
    <Suspense fallback={<CompareSkeleton />}>
      <CompareContent initialPhones={initialPhones} initialVerdict={initialVerdict} initialWinnerId={initialWinnerId} suggestedPairs={suggestedPairs} />
    </Suspense>
  )
}