...
function PhoneDetailInner({ phone, similar, initialFullSpecs }: PhoneDetailClientProps) {
  ...
  const handleCompareToggle = () => {
    setComparePhones(prev => {
      ...
      analytics.compareAddPhone(phone, prev.length + 1)
      return [...prev, phone]
    })
  }
                ...
                <>
                  <a
                    ...
                    onClick={() => analytics.affiliateClick({
                      ...
                      retailer: resolvedOffer?.retailer ?? inferRetailerLabelFromUrl(buyUrl) ?? undefined,
                      price: effectivePrice ?? undefined,
                      location: 'detail_primary',
                    })}
                    target="_blank"
                  ...
                  {hasScopedOffers && alternates.length > 0 && (
                    <AlternateOfferPicker offers={alternates} phone={{ id: phone.id, brand: phone.brand, model_name: phone.model_name }} />
                  )}
                </>
              )}
            </div>
