...
const CACHE = {
  ...
  short:       { next: { revalidate: 300    } } as RequestInit,
  compare:     { next: { revalidate: 300    } } as RequestInit,
  offers:      { next: { revalidate: 600    } } as RequestInit,
  slugs:       { next: { revalidate: 86_400 } } as RequestInit,
} as const
...
type ReqInit = Omit<RequestInit, 'signal'> & { signal?: AbortSignal | null; timeoutMs?: number }
...
export async function resolveComparePhones(
  slugParts: string[],
): Promise<{ phones: Phone[]; verdict: CompareVerdict | null; winnerId: number | null }> {
  const ids = await resolvePhoneIds(slugParts)
  if (ids.length === 0) return { phones: [], verdict: null, winnerId: null }

  try {
    const { phones, verdict, winner_id } = await api.phones.compare(ids)
    const byId = new Map(phones.map(p => [p.id, p]))
    const ordered = ids.map(id => byId.get(id)).filter((p): p is Phone => Boolean(p))
    return { phones: ordered, verdict: verdict ?? null, winnerId: winner_id ?? null }
  } catch {
    return { phones: [], verdict: null, winnerId: null }
  }
}
...
export const api = {
  phones: {
    ...
    slugs: (signal?: AbortSignal) =>
      req<{ phones: PhoneSlugRow[] }>('/phones/slugs', { ...CACHE.slugs, signal }),
    ...
    compareVerdict: (ids: number[], signal?: AbortSignal) =>
      req<CompareVerdictResponse>(
        `/phones/compare/verdict?ids=${ids.join(',')}`,
    ...
    offers: (id: number, region?: string, signal?: AbortSignal) =>
      req<RetailerOffersResponse>(`/phones/${id}/offers${qs({ region })}`, { ...CACHE.offers, signal }),
  },
