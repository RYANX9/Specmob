...
/**
 * The single winner rule. Highest score wins; a tie goes to the lower price,
 * then to the lower id. It depends only on the set of phones, never on the
 * order they were added in. The backend applies the same rule (compare_winner).
 */
export function pickWinnerId(phones: Phone[], scoreOf: (p: Phone) => number): number | null {
  if (phones.length < 2) return null
  let best: Phone | null = null
  for (const p of phones) {
    if (!best) { best = p; continue }
    const diff = scoreOf(p) - scoreOf(best)
    if (diff > 0) { best = p; continue }
    if (diff < 0) continue
    const price = p.price_usd ? p.price_usd : Infinity
    const bestPrice = best.price_usd ? best.price_usd : Infinity
    if (price < bestPrice || (price === bestPrice && p.id < best.id)) best = p
  }
