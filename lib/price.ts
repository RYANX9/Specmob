...
/**
 * Priority: phones.price_usd is already the resolved minimum — variant
 * floor, latest global price_point, or phones.price_usd itself, whichever
 * is lowest — computed server-side by PRICE_RESOLVED_EXPR. This only
 * falls back to the parsed Misc spec text on the rare phone where all
 * three of those are null. historyPoints is intentionally unused here now;
 * kept as a parameter so existing call sites don't need to change.
 */
export function resolveDisplayPrice(phone: Phone, _historyPoints?: PricePointLike[]): number | null {
